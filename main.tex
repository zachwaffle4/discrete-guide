...
\begin{itemize}
    \item $\displaystyle P(x): \sum_{i=1}^{x} i = \frac{i(i+1)}{2}$
    \item $P(x): 6^x + 4$ is a multiple of 5
\end{itemize}
...
\begin{equation*}
P(1): \quad \sum_{i=1}^{1} i = \frac{(1)(1+1)}{2} = \frac{2}{2} = 1 \quad \text{(this is trivially true)}
\end{equation*}

\textbf{Inductive step}: We want to show $P(k) \implies P(k + 1)$

\textit{Inductive hypothesis}: $P(k)$ is true: $\displaystyle \sum_{i=1}^{k} i = 1 + 2 + 3 + \ldots + k = \frac{k(k+1)}{2}$

\vspace{1em}

We want to show $P(k + 1)$ $\displaystyle \sum_{i=1}^{k+1} i = 1 + 2 + 3 + \ldots + k + 1 = \frac{(k+1)(k+2)}{2}$

\vspace{1em}

Start with the definition of $\displaystyle \sum_{i=1}^{k+1} i$ and express the summation including the term preceding $k+1$:

\begin{align*}
\sum_{i=1}^{k+1} i &= 1 + 2 + 3 + \ldots + k + 1 = \quad 1 + 2 + 3 + \ldots + k + (k + 1) \\
&= \frac{k(k+1)}{2} + (k + 1) && \text{(By inductive hypothesis)} \\
&= \frac{k(k+1)}{2} + \frac{2(k + 1)}{2} && \text{(Multiply second term by } 
\frac{2}{2} \text{)} \\
&= \frac{k(k+1)+2(k+1)}{2} && \text{(Add fractions)} \\
&= \frac{k^2+k+2k+2}{2} && \text{(Multiply through)} \\
&= \frac{k^2+3k+2}{2} && \text{(Rearrange)} \\
&= \frac{(k+1)(k+2)}{2} && \text{(Polynomial. Done!)}
\end{align*}
